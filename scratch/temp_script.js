    ...
    // --- FINISHING TOUCHES (Event Handlers) ---
    ...
    window.handleMemberPhotoFileSelect = function(event) {
    ...
    window.clearMemberPhoto = function(event) {
      if (event) {
        event.stopPropagation();
        event.preventDefault();
      }
      const preview = document.getElementById('memberPhotoPreview');
      const input = document.getElementById('memberPhotoFileInput');
      if (preview) {
        preview.src = '';
        preview.style.display = 'none';
      }
      if (input) input.value = '';
      const text = document.querySelector('.photo-placeholder-text');
